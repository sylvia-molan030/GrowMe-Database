...
def _build_block(records: list[dict[str, Any]], label: str, week_label: str) -> dict[str, Any]:
    ...
    notes = {
        "老形式": f"{week_label} 常规上新素材（老命名形式），已计入周度 KPI。",
        "新创意": f"{week_label} 新形式/新创意方向测试（如数字人），已计入周度 KPI。",
...
def get_weekly_test_blocks(week_label: str) -> list[dict[str, Any]]:
    ...
    if weekly_recs:
        blocks.append(_build_block(weekly_recs, "老形式", week_label))
